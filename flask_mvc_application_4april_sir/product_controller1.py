from config1 import app  # instance of flask
...
@app.route("/product")
def welcome_page():
    return render_template('index1.html',flag = False)
...
def add_fresh_product_in_stock():
    ...
    return render_template('product1.html',result = message,d_product = dummy)

@app.route("/product/edit/<int:prid>")
def update_product(prid):
    dbproduct = Product.query.filter_by(id=prid).first()  # id se-- retrive
    if dbproduct:
        return render_template('product1.html', d_product = dbproduct)
...
def delete_products_db(prid):
    ...
    return render_template('list_products1.html', products_list=products) # list
...
def fetch_list_of_products():
    ...
    return render_template('list_products1.html', products_list=products)
...
def search_for_product():
    ...
    return render_template('index1.html',flag = True,products = products)
